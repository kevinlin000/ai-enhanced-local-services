...
ByteBites 不是「會聊天的餐廳推薦機器人」。它把 **AI 推薦 → 訂位 → 訂金付款 → 對話式改單 → 晚到回報 → 商家替代時段 → 補款/退款 → LINE 通知同步** 串成同一條可驗證的交易流程，核心原則只有一句：

```text
AI 負責理解需求與協調流程；Java 後端擁有訂位、付款、事件與退款的業務狀態。
```

模型可以說錯話，但錢和座位的狀態永遠只有一份、永遠由狀態機保護——這是本專案想展示的工程判斷。

## 30 秒 Review Path

| 想快速確認 | 建議入口 |
|---|---|
| 系統樣貌與流程 | 下方「核心流程」與「系統架構」的圖 |
