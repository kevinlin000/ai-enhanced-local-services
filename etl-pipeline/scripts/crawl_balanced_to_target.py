...
# Avoid broad "中式/火鍋" queries here: those categories already dominate.
BALANCED_QUERY_TERMS = (
    "美式漢堡 餐廳",
    "美式早午餐 餐廳",
    "義大利麵 餐廳",
    "披薩 義式餐廳",
...
def main() -> None:
    ...
    parser.add_argument("--target-active", type=int, default=600)
    parser.add_argument("--max-new", type=int, default=220)
    parser.add_argument("--radius", type=int, default=4500)
    ...
    for district, (lat, lng) in TAIPEI_DISTRICTS.items():
        for term in BALANCED_QUERY_TERMS:
            text_query = f"台北 {district} {term}"
    ...
    search_output_path = RAW_DIR / f"places_search_balanced_target{args.target_active}_{timestamp}.json"
    search_output_path.write_text(
        json.dumps(
            {
                "timestamp": timestamp,
                "mode": "balanced_text_search",
    ...
    enriched_output_path = RAW_DIR / f"places_enriched_balanced_target{args.target_active}_{timestamp}.json"
