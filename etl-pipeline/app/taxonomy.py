...
def _load_manual_overrides() -> dict:
    path = Path(__file__).resolve().parents[1] / "data" / "taxonomy" / "manual_overrides.json"
    return json.loads(path.read_text(encoding="utf-8"))


_MANUAL_OVERRIDES = _load_manual_overrides()

MANUAL_AUDIT_PRIMARY_OVERRIDES = tuple(
    (item["match"], item["type_id"])
    for item in _MANUAL_OVERRIDES.get("primary_type_overrides", [])
)
...
MANUAL_NO_KOREAN_TAG_OVERRIDES = tuple(
    item["match"]
    for item in _MANUAL_OVERRIDES.get("suppress_tags", [])
    if "韓式" in item.get("tags", [])
)
